...
class TestToolExec:
    ...
    def _make_tool(self, name, args=None, group_id="G1"):
        from src.warikanbot import Tool
        return Tool(name=name, args=args or {}, group_id=group_id)

    @patch("src.warikanbot.PaymentService")
    def test_start_session(self, MockSvc):
        """start_session → create_session が呼ばれる"""
        mock_svc = MockSvc.return_value
        mock_svc.create_session.return_value = {"status": "success"}
        tool = self._make_tool("start_session", {"name": "飲み会"})
        result = tool.exec()
        mock_svc.create_session.assert_called_once_with("G1", name="飲み会")
        assert result["status"] == "success"

    @patch("src.warikanbot.PaymentService")
    def test_add_payment(self, MockSvc):
        """add_payment → 正しい引数で呼ばれる"""
        mock_svc = MockSvc.return_value
        mock_svc.add_payment.return_value = {"status": "success"}
        tool = self._make_tool("add_payment", {
